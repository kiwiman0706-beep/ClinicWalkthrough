...
* **上部のタブ** — 左が階（1F / 2F / 旧1F）、中央が見方（歩く / 俯瞰 / 平面図）、右が内装・出力の設定パネル
  * **旧1F** はリフォーム前（2008年）の1階です。現行の1F・2Fと同じ 10.8 × 12.5 m のグリッドに
    載っているので、同じ視点・同じ内装で新旧を見比べられます。
...
* **内装** — フロア全体の壁紙・床・受付正面・**天井**の色と床柄。配色プリセットは天井も含めて切り替わります。
* **部屋ごとの仕上げ** — 部屋を選ぶと、その部屋だけの**壁・床・天井**の色と**腰壁**を設定できます。
  * 色を変えた時点でその部屋の個別指定になります（変えていない部屋は全体設定のまま）。
  * **腰壁**はチェックで付き、色と高さ（600〜1,500mm）を変えられます。高さはフロア共通です。
    腰壁のある部屋には**巾木（60mm）と腰見切り（35mm・面から12mm出っ張る）**も一緒に付きます。
    その2色はフロア共通の設定です。
  * **アクセント壁**は「面」と「色」で指定します。面は部屋から見た向き（北＝上／東＝右／南＝下／西＝左）。
    腰壁より上の帯だけが変わります（展開図と同じ考え方です）。
  * 仕切り壁は**面ごと**に仕上がるので、待合側だけ色を変える、といった指定ができます。
  * 「この部屋を全体設定に戻す」「このフロアを全部戻す」で解除します。
  * **「設計の仕様を反映」** — 設計事務所のマテリアル表がある階（現在は2F）に、その配色を一括で当てます。
    2Fは起動時から当たった状態です。解除は「このフロアを全部戻す」。
  * 個別指定は「配置を保存（JSON）」に一緒に入ります。
* **設備（設備図1）** — コンセント・弱電の記号を図面から拾って重ねています。チェックで表示／非表示、
  コンセントと弱電で分けて切り替えられます。一覧には**番号**が振ってあり、平面図にも `#12` の形で出ます。
  取付高さの既定は図面どおり H=250。天井付けなどは番号で指示してください。
...
* 旧1F の患者用WCの隅切り（斜め壁）は図面どおりの角度で入れていますが、床は矩形の重ね合わせで
  近似しています。
